...
import { CREATE_INVOICE, UPDATE_INVOICE } from "@/lib/graphql/mutations";
...
export default function InvoiceForm({ initialData }: { initialData?: any }) {
    ...
    const [createInvoice, { loading: createLoading }] = useMutation<any, any>(CREATE_INVOICE);
    const [updateInvoice, { loading: updateLoading }] = useMutation<any, any>(UPDATE_INVOICE);
    const invoiceLoading = createLoading || updateLoading;

    const [cart, setCart] = useState<CartItem[]>(initialData ? initialData.items.map((i: any) => ({
        productId: i.productId, name: i.productName, sku: i.sku, 
        sellingPrice: i.sellingPrice, costPrice: i.costPrice, gstRate: i.gstRate,
        unit: i.unit, quantity: i.quantity, stock: 9999, useCustomPrice: true, customPrice: i.sellingPrice
    })) : []);
    ...
    const [discount, setDiscount] = useState(initialData?.discountAmount || 0);
    const [paymentMethod, setPaymentMethod] = useState(initialData?.paymentMethod || "CASH");
    const [amountPaid, setAmountPaid] = useState<string>(initialData?.amountPaid ? String(initialData.amountPaid) : "");
    const [gstExempt, setGstExempt] = useState(initialData?.gstExempt || false);
    ...
    useEffect(() => {
        ...
        try {
            ...
            const savedCust = localStorage.getItem("billing_draft_customer");
    ...
    const clearCart = () => {
        ...
        setQtyDraft({});
        if (!initialData) {
            localStorage.removeItem("billing_draft_cart");
        ...
        try {
            ...
            if (initialData) {
                const { data } = await updateInvoice({
                    variables: { input: { ...input, saleId: initialData.saleId } }
                }) as any;
                setSuccess(data.updateInvoice);
            } else {
